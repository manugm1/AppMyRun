package hello.api.estadisticas;

import javax.persistence.Embeddable;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.Table;
import javax.validation.constraints.NotNull;
import java.sql.Timestamp;

/**
 * Created by wilad on 19/05/2016.
 */
@Embeddable
@Entity
@Table(name = "estadisticasexo")

public class EstadisticaSexo {


    public Float getPorcenhombres() {
        return porcenhombres;
    }

    public void setPorcenhombres(Float porcenhombres) {
        this.porcenhombres = porcenhombres;
    }

    @NotNull
    @Id
    private Float porcenhombres;

    public EstadisticaSexo(Float porcenhombres) {
        this.porcenhombres = porcenhombres;
    }
    public EstadisticaSexo() {

    }

    //ESTE METODO DEVUELVE LA HORA DEL SISTEMA ACTUAL COMO UN TIMESTAMP
    public Timestamp getUltfecha() {
        java.util.Date date= new java.util.Date();
        return (new Timestamp(date.getTime()));
    }

    @NotNull
    private Timestamp ultfecha;


}
